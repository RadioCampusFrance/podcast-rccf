...
function load_ecoutes($start, $end, $byEcoute) {
  ...
  if ($byEcoute) {
    $sql = "Select dl_day as day, download, count(*) as nb From log_ecoute where (dl_day >= ".$bdd->quote($start)." and dl_day <= ".$bdd->quote($end).") group by dl_day, download order by count(*) desc;";
  }
  else {
    $datetimeStart = $start . " 00:00:00";
    $datetimeEnd = $end . " 23:59:59";
    $sql = "Select timeslot as day, download, count(*) as nb From log_ecoute where (timeslot >= ".$bdd->quote($datetimeStart)." and timeslot <= ".$bdd->quote($datetimeEnd).") group by timeslot, download order by count(*) desc;";
  }
  ...
  for($i=0; $row = $prep->fetch(); $i++){
    ...
    $result[$row["day"]][$download] = $row["nb"];
  }
...
function get_nb_ecoutes_val($ecoute) {
  $sum = 0;
  if (isset($ecoute[0])) {
    $sum += $ecoute[0];
  }
...
function get_nb_ecoutes($ecoute) {
  $result = "";
  if (isset($ecoute[0])) {
    $result = "".$ecoute[0];
  }
...
<?php 
  ...
  <?php
    ...
    echo '<span class="nombre_ecoutes" title="total (écoutes, téléchargement)">Nombre d\'écoutes</span>';
    if (!$byEcoute) {
	...
	echo " last";
      ...
      echo '" title="'.get_nb_ecoutes($ecoute).'"><strong>'.get_nb_ecoutes_val($ecoute).'</strong></span>';
      $nb = $nb + get_nb_ecoutes_val($ecoute);
      if (!$byEcoute) {
  ...
  ?>
    ...
    <div id="diagrams">
      ...
      <?php } ?>
      <div id="chartdiv2" style="height:500px;width:800px;"></div>
      <?php if (!$byEcoute) { ?>
      <p>On s'intéresse dans le diagramme qui suit au nombre de podcasts écoutés d'un horaire donné.</p>
      <div id="chartdiv3" style="height:500px;width:800px;"></div>
      <?php } else { ?>
      <div id="chartdiv3" style="height:500px;width:800px;"></div>
      <?php } ?>
    </div>
